...
If the server is rebooted, the `runtime` container will almost always enter a crash loop (`Restarting`). You will see a `500 Failed to start bot container` error in the website UI.

**The Cause:** Linux resets the permissions on `/var/run/docker.sock` upon startup. The `runtime` orchestrator needs access to this socket to spawn the headless browser bots, but gets a `Permission denied` error and crashes.

**The Fix (Do NOT rebuild or pull images):** Simply grant permission to the socket and restart the orchestrator.

```bash
sudo chmod 666 /var/run/docker.sock
docker compose -p vexa-v012 -f vexa/deploy/compose/docker-compose.yml restart runtime
```
...
Always use the project's `./update_vexa.sh` script or pin to a specific release tag compatible with the v0.12 line.

#### Step 1: Shut Down and Sync the Submodule

Always pull a specific release tag on the `v012` line, never the main branch.

```bash
# Safely shut down to release file locks
docker compose -p vexa-v012 -f vexa/deploy/compose/docker-compose.yml down

# Fetch all tags directly from the upstream repository
git -C vexa fetch origin --tags

# Checkout the specific version compatible with v012 line
...
Compare your `vexa/.env` with `vexa/deploy/compose/.env.example` and merge any new required variables before restarting. Do not carry forward an older `.env` without adding newly introduced keys.

#### Step 3: Start Vexa

Use the root Makefile to launch services with MeetingMind overrides:
...
`ws://host.docker.internal:18056/ws?api_key=your_key_here`
