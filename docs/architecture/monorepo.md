...
Each parallel slot allocates additional GPU/CPU memory for the KV cache. For `hermes3:8b` (8B), expect roughly **1–2 GB of extra memory per slot**. To adjust the concurrency level, change the `OLLAMA_NUM_PARALLEL` value in `docker-compose.yml`:
