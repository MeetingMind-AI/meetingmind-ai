...
- **Port 8000 Conflicts & "502 Bad Gateway" on Login/Signup**: The backend API binds to host port `8000`. If another application on your system (e.g. another local Python/Django/FastAPI app or container) is already using port `8000`, Docker cannot start `meetingmind_backend`. While the frontend UI (`http://localhost:3000`) will still load, attempting to sign up or log in will fail with `502 Bad Gateway`.
  - **Check running containers**: Run `docker compose ps` and verify that `meetingmind_backend` is in state `Up`.
  - **Identify what is using port 8000**: Run `lsof -i :8000` (macOS/Linux) and terminate the conflicting process (`kill -9 <PID>`).
  - **Or use an alternate port**: Set `BACKEND_PORT=8001` in `.env` and restart (`make app-restart`). Internal communication between frontend and backend within Docker is unaffected.
